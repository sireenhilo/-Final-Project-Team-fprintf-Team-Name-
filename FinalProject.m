...
%%all of our data files and organize the files into Control vs Parkinsonâ€™s Patients.
...
for k = 1:len2 %this loop 
    data2 = importdata(files2(k).name); %imports gait data from Parkinsonâ€™s group
...
for k = 1:len %this loop 
    data = importdata(files2(k).name); %imports gait data from testing group
    data = data(602:1202,[1,end-1,end]); %getting data from 6 to 12 seconds 
    temp = k*ones(length(data),1);
    data = [temp2 data];
    all_test = [all_test ; data]; 
end
